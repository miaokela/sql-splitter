...
class SqlLoader(metaclass=SingletonType):
    ...
    def __init__(self):
        ...
        self.__sql_group = dict()

    @property
    def sqls(self):
        return self.__sql_group

    def preload_all_sqls(self):
        """
        Preloac all sqls into memory
        """
        if not self.SQL_FILE_PATH:
            raise Exception('SQL_FILE_PATH is not configured')
        for _path, _, files in os.walk(self.SQL_FILE_PATH):
            if os.path.samefile(_path, self.SQL_FILE_PATH):  # root path
                for _f in files:
                    ...
                    with open(c_file, "r", encoding="utf-8") as f:
                        content = f.read()
                        __sql_group = yaml.load(content, Loader=yaml.FullLoader)
                        if not __sql_group:
                            continue

                        for _k in __sql_group.keys():
                            _sql_id = "%s.%s" % (c_file_name, _k)
                            self.__sql_group[_sql_id] = __sql_group[_k]
                continue

            relative_path = _path.replace(self.SQL_FILE_PATH, "")
    ...
    def get_sql(self, sql_id: str) -> str:
        if sql_id in self.__sql_group:
            return self.__sql_group[sql_id]
        else:
            sql = self.__load_sql(sql_id)
            self.__sql_group[sql_id] = sql
            return sql
    ...
    def get_sql_data(path: str) -> typing.Dict[str, str]:
        ...
        return {
            file.replace(path if path.endswith(os.sep) else path + os.sep, "")
            .replace(".yml", "")
            .replace(os.sep, "."): file
            for file in file_list
        }

    def get(self, sql_id: str, options: typing.Dict = None) -> str:
        """
        Preloaded sql

        Load SQL statements through sql_id, where sql_id represents {folder}.{file}.{field} or {file}.{field}.
...
if __name__ == "__main__":
    ...
    ret = sql_loader.preload_sql(
        "home_page.sensor.selectAll", options={"equipment_id": 1}
    )
    print(ret)
